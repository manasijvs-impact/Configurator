...
function ProductHierarchy({ scope }) {
  ...
  // Fetch data from bp_product_hierarchy_level table
  useEffect(() => {
    fetchLevels();
  }, []);

  // Helper: Convert label to display name (e.g., "sub_department" -> "Sub Department")
  ...
  const fetchLevels = async () => {
    try {
      ...
      const response = await api.getProductHierarchy();
      console.log('API Response:', response);
      // api.getProductHierarchy returns { success: true, data: [...] }
      // So response.data contains the array
      if (response.data && response.data.length > 0) {
        console.log('Loaded levels:', response.data);
        console.log('First level keys:', Object.keys(response.data[0]));
        console.log('First level:', response.data[0]);
        
        // Process data: if product_hierarchy_level_value is empty but label exists, generate from label
        const processedLevels = response.data.map(level => ({
          ...level,
          product_hierarchy_level_value: level.product_hierarchy_level_value || labelToDisplayName(level.product_hierarchy_level_label) || ''
        }));
      ...
      console.error('Failed to fetch product hierarchy:', err);
  ...
  const generateLevels = () => {
    ...
    for (let i = 0; i < hierarchyCount; i++) {
      newLevels.push({
        ...
        is_competitor_mapping_view_by: false,
        is_strategy_step4_view_by: false,
        is_zone_mapping_view_by: false
      });
    }
    setLevels(newLevels);
    setSetupMode(false);
  ...
  const handleValueChange = (index, value) => {
    ...
    const currentLabel = updated[index].product_hierarchy_level_label || '';
    const currentValue = updated[index].product_hierarchy_level_value || '';
    
    updated[index].product_hierarchy_level_value = value;
    ...
    if (wasAutoGenerated && value.trim()) {
      updated[index].product_hierarchy_level_label = generateLabel(formatDisplayName(value));
    }
  ...
  const handleAddLevel = () => {
    ...
    setLevels([...levels, {
      product_hierarchy_level_id: newId,
      product_hierarchy_level_value: '',
      product_hierarchy_level_label: '',
      is_cascading: false,
  ...
  const handleDeleteLevel = (index) => {
    ...
    newLevels.forEach((level, i) => {
      level.product_hierarchy_level_id = i;
    });
    setLevels(newLevels);
  };
  ...
  const handleSave = async () => {
    ...
    const emptyValues = levels.filter(l => !l.product_hierarchy_level_value.trim());
    ...
    try {
      ...
      const levelsToSave = levels.map(level => ({
        ...level,
        product_hierarchy_level_value: formatDisplayName(level.product_hierarchy_level_value),
        // Preserve manually edited label, or generate from display name if empty
        product_hierarchy_level_label: level.product_hierarchy_level_label?.trim() || generateLabel(level.product_hierarchy_level_value)
      }));

      await api.saveProductHierarchy(levelsToSave);
      setSuccessMessage('Product hierarchy saved successfully!');
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <code style={{ 
            ...
            {schema}.bp_product_hierarchy_level
          </code>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#f5f5f5' }}>
              ...
              <th style={{ ...thStyle, width: '80px', textAlign: 'center' }}>Reporting</th>
              <th style={{ ...thStyle, width: '90px', textAlign: 'center' }}>Zone Mapping</th>
              <th style={{ ...thStyle, width: '100px', textAlign: 'center' }}>Competitor</th>
              <th style={{ ...thStyle, width: '80px', textAlign: 'center' }}>Strategy</th>
              ...
              >
                ...
                <td style={tdStyle}>
                  {labelEditMode ? (
                    <input
                      type="text"
                      value={level.product_hierarchy_level_label || ''}
                      onChange={(e) => updateLevel(index, 'product_hierarchy_level_label', e.target.value)}
                      style={{
                        ...inputStyle,
                        fontFamily: 'monospace'
                      }}
                      placeholder="e.g., sub_department"
                    />
                  ) : (
                    <div style={{
                      padding: '8px 12px',
                      ...
                      {level.product_hierarchy_level_label || '—'}
                    </div>
                  )}
                </td>
                <td style={{ ...tdStyle, textAlign: 'center' }}>
                ...
                <td style={{ ...tdStyle, textAlign: 'center' }}>
                  <input
                    type="checkbox"
                    checked={level.is_zone_mapping_view_by || false}
                    onChange={(e) => updateLevel(index, 'is_zone_mapping_view_by', e.target.checked)}
                    style={{ width: '18px', height: '18px', cursor: 'pointer' }}
                    title="Available in Zone Mapping view?"
                  />
                </td>
                <td style={{ ...tdStyle, textAlign: 'center' }}>
                  <input
                    type="checkbox"
                    checked={level.is_competitor_mapping_view_by || false}
                    onChange={(e) => updateLevel(index, 'is_competitor_mapping_view_by', e.target.checked)}
                    style={{ width: '18px', height: '18px', cursor: 'pointer' }}
                    title="Available in Competitor Mapping view?"
                  />
                </td>
                <td style={{ ...tdStyle, textAlign: 'center' }}>
                  <input
                    type="checkbox"
                    checked={level.is_strategy_step4_view_by || false}
                    onChange={(e) => updateLevel(index, 'is_strategy_step4_view_by', e.target.checked)}
                    style={{ width: '18px', height: '18px', cursor: 'pointer' }}
                    title="Available in Strategy Step 4 view?"
                  />
                </td>
                <td style={{ ...tdStyle, textAlign: 'center' }}>
                  <button 
...
const inputStyle = { width: '100%', minWidth: '120px', padding: '8px 12px', borderRadius: '6px', border: '1px solid #d0d0d0', fontSize: '14px', boxSizing: 'border-box' };
