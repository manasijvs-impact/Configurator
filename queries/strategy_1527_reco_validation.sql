...
-- ============================================
-- VALIDATION QUERY (ACTUALS + FORECAST)
-- Strategy ID: 1527
-- Schema: base_pricing
...
actuals_cutoff AS (
    SELECT COALESCE(
        (SELECT MAX(t.transaction_date) 
         FROM base_pricing.bp_transaction_data_daily t
         CROSS JOIN strategy_params spr
...
actuals_granular AS (
    ...
    FROM bins_mapping bm
    INNER JOIN base_pricing.bp_transaction_data_daily t
...
sim_week AS (
    ...
    CROSS JOIN strategy_params sp
    CROSS JOIN actuals_cutoff ac
    WHERE w.week_start_date > ac.max_actuals_date - INTERVAL '6 days' AND w.week_start_date <= sp.end_date
),
...
promo_week_store AS (
    ...
    CROSS JOIN strategy_params spr
    CROSS JOIN actuals_cutoff ac
    WHERE p.week_start_date > ac.max_actuals_date - INTERVAL '6 days' AND p.week_start_date <= spr.end_date
),
...
day_split_kvi_raw AS (
    ...
    CROSS JOIN strategy_params sp
    CROSS JOIN actuals_cutoff ac
    WHERE pcs.is_kvi AND d.date > ac.max_actuals_date AND d.date <= sp.end_date
    GROUP BY d.product_id, d.channel_id, pcs.segment_id, d.segment_id, d.week_start_date
),
...
day_split_non_kvi_raw AS (
    ...
    CROSS JOIN strategy_params sp
    CROSS JOIN actuals_cutoff ac
    WHERE NOT pcs.is_kvi AND d.date > ac.max_actuals_date AND d.date <= sp.end_date
    GROUP BY pcs.product_id, d.channel_id, pcs.segment_id, d.segment_id, d.week_start_date
),
...
store_split_kvi_raw AS (
    ...
    CROSS JOIN strategy_params spr
    CROSS JOIN actuals_cutoff ac
    WHERE sph.is_kvi AND ss.week_start_date > ac.max_actuals_date - INTERVAL '6 days' AND ss.week_start_date <= spr.end_date
),
...
store_split_non_kvi_raw AS (
    ...
    CROSS JOIN strategy_params spr
    CROSS JOIN actuals_cutoff ac
    WHERE NOT sph.is_kvi AND ss.week_start_date > ac.max_actuals_date - INTERVAL '6 days' AND ss.week_start_date <= spr.end_date
),
